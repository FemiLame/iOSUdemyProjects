...
struct StoryBrain {
    ...
    mutating func nextStory(_ userChoice: String) {
        if (userChoice == stroies[currentStory].choice1) {
            currentStory = stroies[currentStory].choice1Destination
        } else {
            currentStory = stroies[currentStory].choice2Destination
        }
    }
    
    private func getCurrentStory() -> Story {
        return stroies[currentStory]
    }
